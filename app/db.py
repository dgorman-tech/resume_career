...
def _ensure_profile_columns(conn):
    ...
    if "holistic_weight" not in cols:
        conn.execute("ALTER TABLE profile ADD COLUMN holistic_weight INTEGER NOT NULL DEFAULT 50")
    if "rubric_updated_at" not in cols:
        conn.execute("ALTER TABLE profile ADD COLUMN rubric_updated_at TEXT")
